(() => {
  ...
  function initMenuMobile() {
    ...
    const alternar = (abrir) => {
      ...
      botao.setAttribute(
        'aria-label',
        abrir ? 'Fechar menu de navegação' : 'Abrir menu de navegação',
      );
  ...
  function criarCarrossel(raiz) {
    ...
    const atualizar = () => {
      ...
      if (dotsBox)
        $$('.carrossel__dot', dotsBox).forEach((d, i) =>
          d.classList.toggle('is-ativo', i === indice),
        );
    };
  ...
  function initCarrosseis() {
    $$('[data-carrossel]').forEach(criarCarrossel);
  }
  ...
  function initAbertura() {
    ...
    if (brasao) {
      ...
      if (!brasao.getAttribute('src') || (brasao.complete && brasao.naturalWidth === 0))
        semBrasao();
    }
    ...
    const T = rm ? { aba: 20, sai: 120, fim: 260 } : { aba: 650, sai: 2300, fim: 3250 };
  ...
  function initPresentes() {
    ...
    $$('[data-cota]').forEach((card) => {
      ...
      $$('[data-cota-pagar]', card).forEach((b) =>
        b.addEventListener('click', () => abrir(nome, valor, link)),
      );
    });
